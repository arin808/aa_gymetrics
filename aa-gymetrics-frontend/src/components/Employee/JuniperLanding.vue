...
import getAllJuniper from "@/api/actives/all-gym/get-all-Juniper";
...
export default {
  ...
  //State property of loggedIn for access control

  //Mount get result to show all active students in the gym
  async mounted() {
    this.students = await getAllJuniper();
  },
  methods: {
    //Method for check in
    async checkIn() {
      ...
      if (student) {
        ...
        //Process check in
        const response = await checkIn(student, "Juniper");
