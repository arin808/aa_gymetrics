...
import getAllChaparral from "@/api/actives/all-gym/get-all-Chaparral";
...
export default {
  ...
  async mounted() {
    this.students = await getAllChaparral();
  },
  methods: {
    //Method for check in
    async checkIn() {
      ...
      if (student) {
        ...
        if (existing) {
          ...
          if (existing.gymLocation == "Chaparral") {
        ...
        //process check in
        const response = await checkIn(student, "Chaparral");
