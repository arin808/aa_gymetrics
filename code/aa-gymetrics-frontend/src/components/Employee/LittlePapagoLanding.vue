...
import getAllLittlePapago from "@/api/actives/all-gym/get-all-LittlePapago";
...
export default {
  ...
  async mounted() {
    this.students = await getAllLittlePapago();
  },
  methods: {
    //Method for check in
    async checkIn() {
      ...
      if (student) {
        ...
        if (existing) {
          ...
          if (existing.gymLocation == "Little-Papago") {
        ...
        //process check in
        const response = await checkIn(student, "Little-Papago");
